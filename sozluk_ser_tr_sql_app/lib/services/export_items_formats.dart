// <📜 ----- export_items_formats.dart ----->
/*
  🧩 Sırpça dışa aktarma format yardımcıları (CSV, JSON & XLSX)
...
String buildWordsCsvNoId(List<Word> list) {
  final headers = ['Sırpça', 'Türkçe'];
  final sb = StringBuffer();
  sb.write('\uFEFF'); // Excel uyumu için BOM
  sb.writeln(headers.map(_csvEscape).join(','));

  for (final w in list) {
    final row = [w.sirpca, w.turkce].map(_csvEscape).join(',');
    sb.writeln(row);
  }
  return sb.toString();
}
