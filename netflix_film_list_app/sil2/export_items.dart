...
Future<ExportItems> exportItemsToFileFormats({
  ...
  try {
    ...
    final List<NetflixItem> all = await DbHelper.instance.getRecords();

    // 2) Alfabetik sırala
    all.sort(
      (a, b) => a.netflixItemName.toLowerCase().compareTo(
        b.netflixItemName.toLowerCase(),
      ),
    );

    // 3) JSON (pretty) — ID YOK
    final jsonList = all
        .map((w) => {'word': w.netflixItemName, 'meaning': w.watchDate})
        .toList();
