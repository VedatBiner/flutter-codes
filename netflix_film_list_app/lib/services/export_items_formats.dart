...
/*
  🧩 Word dışa aktarma format yardımcıları (CSV, JSON & XLSX)

  NE YAPAR?
  - buildWordsCsvNoId:  List<Word> → UTF-8 BOM 'lu CSV (başlıklar: kelime,anlam)
...
String buildWordsCsvNoId(List<NetflixItem> list) {
  ...
  for (final w in list) {
    final row = [w.netflixItemName, w.watchDate].map(_csvEscape).join(',');
    sb.writeln(row);
  }
  return sb.toString();
}
...
Uint8List buildWordsXlsxNoId(List<NetflixItem> list) {
  final wb = xlsio.Workbook();
  final sheet = wb.worksheets[0];

  // Başlıklar
  final headers = ['netflixItemName', 'watchDate'];
  ...
  for (int r = 0; r < list.length; r++) {
    final w = list[r];
    sheet.getRangeByIndex(r + 2, 1).setText(w.netflixItemName);
    sheet.getRangeByIndex(r + 2, 2).setText(w.watchDate);
  }
