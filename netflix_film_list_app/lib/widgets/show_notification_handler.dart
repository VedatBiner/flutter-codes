// 📃 <----- show_notification_handler.dart ----->
...
///
void showDeleteNotification(BuildContext context, NetflixItem netflixItem) {
  return NotificationService.showCustomNotification(
    context: context,
    ...
    message: RichText(
      text: TextSpan(
        children: [
          TextSpan(text: netflixItem.netflixItemName, style: kelimeText),
...
///
void showUpdateNotification(BuildContext context, NetflixItem updated) {
  return NotificationService.showCustomNotification(
    context: context,
    ...
    message: RichText(
      text: TextSpan(
        children: [
          TextSpan(text: updated.netflixItemName, style: kelimeAddText),
          const TextSpan(
...
///
void showAddNotification(BuildContext context, NetflixItem result) {
  return NotificationService.showCustomNotification(
    context: context,
    ...
    message: RichText(
      text: TextSpan(
        children: [
          TextSpan(text: result.netflixItemName, style: kelimeAddText),
          const TextSpan(
            text: ' adlı dizi/film eklendi.',
            style: normalBlackText,
          ),
        ],
      ),
...
///
void showExistNotification(BuildContext context, NetflixItem result) {
  return NotificationService.showCustomNotification(
    context: context,
    title: 'Uyarı Mesajı',
    message: RichText(
      text: TextSpan(
        children: [
          TextSpan(text: result.netflixItemName, style: kelimeExistText),
...
Future<void> showWordDialogHandler(
  ...
  final result = await showDialog<NetflixItem>(
    context: context,
  ...
  if (result != null) {
    final existing = await DbHelper.instance.getItem(result.netflixItemName);
